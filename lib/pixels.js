...
//
// Deux détections voisines mais pas jointives laissent un trou entre elles,
// et l'ensemble ressort en morceaux là où le terrain a brûlé d'un seul tenant.
// La fermeture — dilatation puis érosion du même rayon — comble ces trous
// sans faire grossir le contour extérieur : ce qui était déjà plein le reste,
// ce qui était un interstice se remplit.
//
// La dilatation prend le maximum sur une fenêtre carrée, donc l'horodatage le
// plus récent du voisinage : une cellule comblée hérite de l'activité qui l'a
// comblée, ce qui la place dans le bon palier de couleur.
//
// Les fenêtres glissantes sont calculées en temps constant par cellule (file
// à double extrémité conservant les maxima candidats), sinon le coût serait
// proportionnel au rayon et deviendrait le poste dominant sur des millions de
// cellules rejouées à chaque cran de la barre temporelle.

// `horsGrille` est la valeur supposée au-delà des bords. Elle vaut 0 pour la
// dilatation (rien à ajouter hors de la grille) mais 1 pour l'érosion, menée
// sur le complémentaire : le vide commence là où la grille s'arrête, sans
// quoi une forme touchant le bord ne serait jamais rongée de ce côté.
function maxFenetre(src, dst, largeur, hauteur, r, file, horizontal, horsGrille) {
  const nb = horizontal ? hauteur : largeur;
  const longueur = horizontal ? largeur : hauteur;
  const pas = horizontal ? 1 : largeur;
  for (let n = 0; n < nb; n++) {
    const base = horizontal ? n * largeur : n;
    let tete = 0, queue = 0;
    for (let k = 0; k < longueur + r; k++) {
      if (k < longueur) {
        const v = src[base + k * pas];
        while (queue > tete && src[base + file[queue - 1] * pas] <= v) queue--;
        file[queue++] = k;
      }
      const centre = k - r;
      if (centre >= 0) {
        while (file[tete] < centre - r) tete++;
        let m = src[base + file[tete] * pas];
        // Fenêtre débordant de la grille : le dehors compte comme une valeur.
        if (horsGrille && (centre < r || centre >= longueur - r) && horsGrille > m) m = horsGrille;
        dst[base + centre * pas] = m;
      }
    }
  }
}
...
/**
 * Ferme les trous de rayon inférieur à `r` cellules dans une grille de
 * valeurs, et écrit le résultat dans `atelier.sortie`. `atelier` est réutilisé
 * d'un appel à l'autre pour ne pas réallouer à chaque cran.
 */
function creerAtelierFermeture(total, largeur, hauteur) {
  return {
    tampon: new Uint32Array(total),
    sortie: new Uint32Array(total),
    vide: new Uint8Array(total),
    videTampon: new Uint8Array(total),
    file: new Int32Array(Math.max(largeur, hauteur)),
  };
}

function fermerTrous(valeurs, largeur, hauteur, r, atelier) {
  if (r < 1) return valeurs;
  const { tampon, sortie, vide, videTampon, file } = atelier;

  // Dilatation : maximum sur la fenêtre, en deux passes séparables.
  maxFenetre(valeurs, tampon, largeur, hauteur, r, file, true, 0);
  maxFenetre(tampon, sortie, largeur, hauteur, r, file, false, 0);

  // Érosion du dilaté. Éroder revient à dilater le complémentaire : une
  // cellule ne survit que si aucune cellule vide ne se trouve dans sa
  // fenêtre. C'est ce qui empêche le contour extérieur de gonfler.
  for (let n = 0; n < sortie.length; n++) vide[n] = sortie[n] === 0 ? 1 : 0;
  maxFenetre(vide, videTampon, largeur, hauteur, r, file, true, 1);
  maxFenetre(videTampon, vide, largeur, hauteur, r, file, false, 1);

  for (let n = 0; n < sortie.length; n++) if (vide[n]) sortie[n] = 0;

  // Une cellule réellement détectée garde sa propre date, et ne disparaît
  // jamais — l'érosion ronge les bords de la dilatation, y compris là où il y
  // avait du vrai.
  //
  // Le rétablissement est une affectation, pas un maximum. La dilatation prend
  // le maximum sur la fenêtre, donc la date la plus récente du voisinage : à
  // rayon de fermeture large, un sol brûlé il y a trois jours héritait de la
  // date d'un foyer actif à deux kilomètres de là et se repeignait en rouge.
  // Seules les cellules ajoutées par la fermeture prennent une date interpolée.
  for (let n = 0; n < valeurs.length; n++) if (valeurs[n]) sortie[n] = valeurs[n];
  return sortie;
}
...
module.exports = {
  ...
  fermerTrous, creerAtelierFermeture,
