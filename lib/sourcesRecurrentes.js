...
// Diagnostic : combien de cellules ont déjà atteint tel nombre de jours
// distincts. Le filtre réel (sourcesConnues, plus bas) ne s'appuie pas sur
// ces seuils — il se déclenche sur la date de première détection, pas sur
// un total de jours — mais ça reste utile pour suivre l'accumulation.
...
// En dessous, une seule détection hors saison ne suffit pas : un pixel isolé
// (bruit satellite, reflet, écobuage ponctuel — le brûlage agricole hivernal
// est légal et fréquent en France, sans rapport avec un feu de forêt) ne
// doit pas exclure définitivement un lieu où un vrai feu pourrait démarrer
// plus tard. Une source industrielle, elle, s'allume plus d'une fois.
const JOURS_MINI_SOURCE = 2;
...
// précédent, où un feu de forêt est quasiment impossible en France — ET
// elle a été vue au moins JOURS_MINI_SOURCE fois (voir plus haut).
//
// La date, pas un compte de jours élevé, reste le critère principal : un
// compte élevé confondrait à tort un très gros feu d'été avec une source
// industrielle (voir l'en-tête du fichier). Une cellule vue hors saison ne
// peut pas être ce feu-ci — il n'avait pas encore démarré.
async function sourcesConnues(redis, avantDate) {
  ...
  Object.keys(brut || {}).forEach((cle) => {
    const [nbJours, premierJour] = brut[cle].split(':');
    if (premierJour && premierJour <= avantDate && Number(nbJours) >= JOURS_MINI_SOURCE) connues.add(cle);
  });
  return connues;
}
