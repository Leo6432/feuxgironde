...
// v3 : le contrôle SOMMETS_MIN a changé de comportement (voir plus bas) ; sans
// changer la clé, une ancienne géométrie trop simplifiée déjà en cache depuis
// v2 ne serait jamais revérifiée ni re-téléchargée avant 30 jours.
const CLE_CACHE = 'geo:france:v3';
const DELAI_MS = 5000;
...
async function frontiereFrance(redis) {
  ...
  if (redis) {
    try {
      ...
      if (brut) {
        const geometrieEnCache = JSON.parse(brut);
        // Le cache n'est digne de confiance que s'il respecte les mêmes
        // règles que le téléchargement — sinon une géométrie posée là par une
        // ancienne version du code (avant ce contrôle) resterait servie
        // jusqu'à expiration, sans jamais être revérifiée.
        if (compterSommets(geometrieEnCache) >= SOMMETS_MIN) {
          enMemoire = geometrieEnCache;
          return enMemoire;
        }
      }
