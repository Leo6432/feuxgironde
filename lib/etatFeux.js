...
// Plafond de sommets pour un état.
//
// Conserver deux mois de terrain brûlé (lib/histoDetections.js) multiplie les
// contours : mesuré, un été très chargé dépassait les 4,5 Mo qu'une fonction
// Vercel peut renvoyer — et de toute façon ce que Leaflet redessine sans
// à-coups. Quand le compte déborde, on élargit la maille de tous les foyers
// d'un cran et on recommence : les pixels grossissent, aucune zone ne s'en va.
const MAX_SOMMETS_ETAT = 150000;

// Prépare les foyers et calcule les zones à `instant`, à la maille la plus
// fine qui tienne dans ce plafond. Renvoie de quoi enchaîner : le précalcul
// réutilise le `rangMini` retenu pour tous ses crans, sur les mêmes grilles.
function etatAuBudget(points, instant) {
  let tentative = null;
  let rangMini = 0;
  while (rangMini < PAS_M_LADDER.length) {
    ...
    tentative = { rangMini, prepare, zones, cellulesTotal, sommets: compterSommets(zones) };
    if (tentative.sommets <= MAX_SOMMETS_ETAT) break;

    // Le nombre de sommets suit le périmètre divisé par la maille, et chaque
    // cran de l'échelle double la maille : le dépassement dit donc directement
    // combien de crans passer. Essayer un cran à la fois coûtait jusqu'à cinq
    // passes complètes de marquage — une trentaine de secondes sur un été
    // chargé, pour un résultat identique.
    const bonds = Math.max(1, Math.ceil(Math.log2(tentative.sommets / MAX_SOMMETS_ETAT)));
...
module.exports = {
  ...
  etatAuBudget, compterSommets, MAX_SOMMETS_ETAT,
