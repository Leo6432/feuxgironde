...
// avions civils ordinaires (F-G). C'est un filtrage BEST-EFFORT, revu sur
// preuve à mesure des avis :
//   — « SECU » : un premier relevé réel (1766 avions au-dessus de la France,
//     aucun ne correspondait aux motifs devinés au départ) faisait ressortir
//     « SECUL », qui n'avait pas la forme d'un vol commercial (ceux-ci
//     suivent presque tous « code compagnie + numéro de vol », ex. RYR3BJ,
//     TAP1337 — « SECUL » non).
//   — « MILAN » : observé DIRECTEMENT par l'utilisateur sur un site de
//     référence — un Dash 8 Q400 (F-ZBMK, préfixe d'État) affichait
//     l'indicatif « MILAN78 ». La supposition initiale, « DASH », était donc
//     fausse ; retirée.
//   — « MORA » : même origine — un hélicoptère EC145 (F-ZBQW, préfixe
//     d'État lui aussi) affichait « MORA832 ». Cet appareil sert surtout aux
//     évacuations sanitaires et à la reconnaissance, pas au largage d'eau,
//     mais il reste un moyen aérien de la Sécurité civile engagé en soutien
//     des feux — donc à sa place ici.
...
// un feu plutôt que d'un vol privé, d'école ou de tourisme sans rapport. Un
// Écureuil sous contrat pour larguer de l'eau est plausible (courant dans le
// Sud en saison), mais l'immatriculation seule ne permet pas de le
// distinguer d'un Écureuil qui ne l'est pas — l'ajouter au filtre ferait
// passer n'importe quel hélicoptère privé de passage pour un moyen de lutte.
//
...
// ── Proximité d'un feu actif ────────────────────────────────────────────
//
// Appartenir à la flotte de la Sécurité civile ne dit pas ce que l'avion
// fait à l'instant : un EC145 (MORA) vole toute l'année pour du secours en
// montagne, un Canadair peut voler pour l'entraînement ou un simple
// repositionnement entre bases. Rien dans la position seule ne dit « ceci est
// une mission feu ». Le signal le plus honnête disponible est la proximité
// avec un foyer que le site suit déjà lui-même : pas de feu actif à
// proximité, pas d'avion affiché — quel que soit son type.
//
// Les détections actives sont relues depuis l'archive déjà calculée par le
// moteur de feux (voir lib/etatFeux.js), jamais recalculées ici : cette
// fonction ne doit pas déclencher une nouvelle moisson FIRMS, seulement lire
// ce qui est déjà en cache.
const etatFeux = require('./etatFeux');

// Généreux plutôt que strict : un avion en approche ou faisant l'aller-retour
// vers un plan d'eau proche reste à quelques dizaines de kilomètres du foyer,
// pas dessus. Trop serré le ferait disparaître entre deux passes.
const RAYON_FEU_KM = 50;
...
function presDunFeu(avion, foyersActifs) {
  return foyersActifs.some((f) => distanceKm(avion.lat, avion.lon, f[0], f[1]) <= RAYON_FEU_KM);
}

// Détections actives connues (dernier cran archivé), sous la forme
// [lat, lon, frp, ts] — voir foyersActifs() dans lib/etatFeux.js, c'est
// exactement ce champ qui est relu ici. Toute défaillance (pas d'archive
// encore calculée, Redis indisponible...) rend une liste vide plutôt que de
// faire échouer l'appelant : sans feu connu, aucun avion ne doit s'afficher,
// ce qui est déjà le comportement voulu.
async function foyersActifsConnus(redis) {
  if (!redis) return [];
  try {
    const dernier = etatFeux.instantsDisponibles(Date.now()).slice(-1)[0];
    const rang = await etatFeux.lireRangMaille(redis);
    if (rang === null) return [];
    const etat = await etatFeux.lireEtat(redis, dernier, rang);
    return (etat && Array.isArray(etat.actifs)) ? etat.actifs : [];
  } catch (e) {
    return [];
  }
}

// Ne garde que les avions à proximité d'un feu actif connu. Sans aucun feu
// actif connu, la liste vide n'est pas un défaut de cette fonction : c'est le
// résultat demandé — hors saison ou la nuit, aucun avion ne doit apparaître.
async function filtrerPresDesFeux(avions, redis) {
  const foyers = await foyersActifsConnus(redis);
  if (!foyers.length) return { avions: [], foyersActifs: 0 };
  return { avions: avions.filter((a) => presDunFeu(a, foyers)), foyersActifs: foyers.length };
}

module.exports = {
  recupererAvions, estAvionFeu, acVersAvion, estIndicatifInsolite, PATRONS,
  LAMIN, LAMAX, LOMIN, LOMAX, RAYON_NM, CENTRES,
  filtrerPresDesFeux, foyersActifsConnus, presDunFeu, distanceKm, RAYON_FEU_KM,
};
