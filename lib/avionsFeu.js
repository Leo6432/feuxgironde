...
// transpondeur. C'est un filtrage BEST-EFFORT, et le premier jeu de motifs
// (PELICAN / FZB / DASH) s'est révélé faux : sur un relevé réel de 1766 avions
// au-dessus de la France, aucun ne correspondait. Un indicatif, « SECUL »,
// est ressorti du lot — il ne collait à aucun des trois motifs mais avait
// clairement la forme d'un indicatif d'État plutôt que d'un vol commercial
// (ceux-ci suivent presque tous le format « code compagnie + numéro de vol »,
// ex. RYR3BJ, TAP1337 — « SECUL » n'a pas cette forme). D'où « SECU ».
//   — « SECU » : plausible pour « Sécurité civile », vu en direct.
//   — « PELICAN » : indicatif documenté par ailleurs pour les Canadair
//     CL-415, mais absent de ce relevé — soit aucun n'était en vol à ce
//     moment, soit l'indicatif réel diffère. Gardé par précaution.
//   — « F-ZB » (sans tiret) : bloc d'immatriculation de la Direction
//     générale de la sécurité civile, en repli si un avion diffuse son
//     immatriculation plutôt qu'un indicatif dédié.
//   — « DASH » : le plus incertain des quatre, gardé pour la même raison.
// Si le relevé suivant (voir estIndicatifInsolite) fait ressortir d'autres
// candidats, PATRONS est l'endroit à corriger — un seul tableau.
const PATRONS = [/^SECU/, /^PELICAN/, /^FZB/, /^DASH/];
