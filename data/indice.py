...
from meteo import JOURS, DERNIERE_PLUIE
...
def composantes(heures, i):
    """Les 5 composantes 0-100 pour l'heure i."""
    h, t, hum, moy, raf, pres = heures[i]
    # Instabilité : chute de pression sur les 6 heures précédentes.
    j = max(0, i - 6)
...
def score_horaire(heures, i):
    c = composantes(heures, i)
...
def analyse(nom, jour_num):
    ...
    scores = [score_horaire(heures, i) for i in range(len(heures))]
    ...
    return {"nom":nom, "modele":d["modele"], "score":final, "pire_heure":heures[pire_i][0],
            ...
            "comp":composantes(heures, pire_i), "periodes":periodes}
