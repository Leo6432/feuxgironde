...
META = {
 "Mar 28": dict(num=28, id="mar28", tag="En cours",         tagc="tag-red",  risque=40, lvl="mid",
                proj="42 000 – 51 000 ha", peak=False),
 "Mer 29": dict(num=29, id="mer29", tag="Fenêtre 17h–19h",  tagc="tag-red",  risque=75, lvl="hi",
                proj="50 000 – 68 000 ha", peak=True),
 "Jeu 30": dict(num=30, id="jeu30", tag="Prévision ARPEGE", tagc="tag-blue", risque=10, lvl="lo",
                proj="Accalmie, mais toujours aucune pluie · 51 000 – 69 000 ha", peak=False),
 "Ven 31": dict(num=31, id="ven31", tag="Prévision ARPEGE", tagc="tag-blue", risque=8,  lvl="lo",
...
for nom, m in META.items():
    ...
    out.append(f'''          <tr{tr}>
            ...
            <td class="num lvl lvl-{niveau(s)}" data-label="Difficulté">{s}</td>
          ...
          <tr class="day-detail" id="{m['id']}" hidden>
            <td colspan="7">
              <div class="periodes">''')
    for p in r["periodes"]:
